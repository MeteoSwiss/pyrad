...
def process_trajectory(procstatus, dscfg, radar_list=None, trajectory=None):
    ...
    if not dscfg["initialized"]:
        if trajectory is None:
            raise Exception(
                "ERROR: Undefined trajectory for dataset '%s'" % dscfg["dsname"]
            )
...
def process_traj_trt(procstatus, dscfg, radar_list=None, trajectory=None):
    ...
    if radar.ray_angle_res is not None:
        radar_roi.ray_angle_res["data"] = np.array([radar.ray_angle_res["data"][0]])
...
def process_traj_trt_contour(procstatus, dscfg, radar_list=None, trajectory=None):
    ...
    roi_dict = _get_contour_trt(radar, trajectory, dscfg["timeinfo"], time_tol=time_tol)

    if roi_dict is None:
        return None, None

    new_dataset = {"radar_out": radar, "roi_dict": roi_dict}

    return new_dataset, ind_rad


def process_traj_lightning(procstatus, dscfg, radar_list=None, trajectory=None):
    """
    ...
    if procstatus == 2:
        ...
        dataset = {"ts": trajdict["ts"], "final": 1}
    ...
    if not dscfg["initialized"]:
        ...
        for datatype, field_name in zip(datatypes, field_names):
            ts = TimeSeries(
                description, maxlength=trajectory.time_vector.size, datatype=datatype
            )
    ...
    traj_time_vec = date2num(
        trajectory.time_vector, radar.time["units"], radar.time["calendar"]
    )
    ...
    az_tol = dscfg.get("az_tol", 3.0)
    el_tol = dscfg.get("el_tol", 3.0)
    ...
    for tind in np.nditer(traj_ind):
        ...
        for field_name in field_names:
            ...
            cell_vals = rdata[cell_ray_inds, cell_rng_ind_min : cell_rng_ind_max + 1]
            ...
            if data_is_log[field_name]:
                ...
                if cell_vals_valid.size > 0:
                    vals_lin = np.ma.power(10.0, cell_vals_valid / 10.0)
                    val_mean = np.ma.mean(vals_lin)
                    val_mean = 10.0 * np.ma.log10(val_mean)
            else:
...
def process_traj_atplane(procstatus, dscfg, radar_list=None, trajectory=None):
    ...
    if procstatus == 2:
        ...
        dataset = {"ts_dict": trajdict["ts_dict"], "final": 1}
        return dataset, trajdict["ind_rad"]
    ...
    if not dscfg["initialized"]:
        ...
        for datatype, field_name in zip(datatypes, field_names):
            ts = TimeSeries(
                ...
                maxlength=trajectory.time_vector.size,
                datatype=datatype,
            )
...
def process_traj_antenna_pattern(procstatus, dscfg, radar_list=None, trajectory=None):
    ...
    if procstatus == 2:
        ...
        dataset = {"ts_dict": tadict["ts_dict"], "final": 1}
    ...
    if not _get_ts_values_antenna_pattern(
        radar, trajectory, tadict, traj_ind, field_names, az_tol=az_tol, el_tol=el_tol
    ):
        return None, None
...
def _get_ts_values_antenna_pattern(
    radar, trajectory, tadict, traj_ind, field_names, az_tol=3.0, el_tol=3.0
):
    """
...
def _get_contour_trt(radar, trajectory, voltime, time_tol=100.0):
    """
...
def _get_gates(radar, az, el, rr, tt, trajdict, ang_tol=1.2, az_tol=3.0, el_tol=3.0):
    """
    ...
    if not cell_center:
        ...
        for sweep in range(radar_ppi.nsweeps):
            ...
            ind_ray, ind_rng, _, _ = find_nearest_gate(
                radar_aux,
                trajectory.wgs84_lat_deg[ind],
                trajectory.wgs84_lon_deg[ind],
                latlon_tol=latlon_tol,
            )
            ...
            lat = np.append(lat, radar_aux.gate_latitude["data"][ind_ray][ind_rng])
            lon = np.append(lon, radar_aux.gate_longitude["data"][ind_ray][ind_rng])
            alt = np.append(alt, radar_aux.gate_altitude["data"][ind_ray][ind_rng])
...
def _get_gates_antenna_pattern(
    radar_sel,
    target_radar,
    az,
    rr,
    ...
    r_radar = Radar(
        ...
        None,
        "rhi",
        target_radar.latitude,
        target_radar.longitude,
    ...
    radar_sel.add_field("colocated_gates", gate_coloc_radar_sel, replace_existing=True)
...
def _get_closest_bin(az, el, rr, tt, radar, tdict):
    ...
    if tdict["radar_old"] is not None:
        ...
        # Find closest time
        if (dt_old < 0.0) and (tdict["radar_old2"] is not None):

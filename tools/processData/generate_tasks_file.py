...
def main():
    """ """
    print(
        "====== tasks file generation started: %s"
        % datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%d %H:%M:%S")
    )
    atexit.register(_print_end_msg, "====== tasks file generation finished: ")
    ...
    if cpus_t > max_cpus:
        warn(
            "Number of CPUs to use to large. Available="
            + str(max_cpus)
            + " Required="
            + str(cpus_t)
        )
    ...
    cfgfiles = "cscs_rad4alp_gc_freq_PHW.txt"
    radar = "W"
    res = "H"
    info = "W"
